...
public class Cycle_Undirected_Graph_BFS {
    ...
    */
    ...
    private static boolean isCycle(int V, ArrayList<ArrayList<Integer>> adj, boolean[] vis) {
        for (int i = 1; i <= V; ++i) {
            if (!vis[i]) {
                if (checkForCycle_BFS(i, adj, vis)) {
                    return true;
                }
            }
        }
    ...
    private static boolean checkForCycle_BFS(int s, ArrayList<ArrayList<Integer>> adj, boolean[] vis) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("The Undirected Graph has Cycle using BFS: " + solve(A, B));
    }
}
