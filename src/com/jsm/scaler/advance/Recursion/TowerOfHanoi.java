...
public class TowerOfHanoi {
    ...
    */
    ...
    private static void toh(int disk, int source, int helper, int dest) {
        if (disk == 0)
            return;
        toh(disk - 1, source, dest, helper);
        result[move++] = new int[]{disk, source, dest};
        toh(disk - 1, helper, source, dest);
    }

    public static void main(String[] args) {
        int A = 3;
        System.out.println(Arrays.deepToString(towerOfHanoi(A)));
    }
}
